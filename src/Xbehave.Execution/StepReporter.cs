...
namespace Xbehave.Execution
{
    ...
    public class StepReporter : IStepReporter
    {
        ...
        {
            this.reporter = new Reporter(messageBus, cancellationTokenSource);
            this.scenario = scenario;
        }

        public void Begin(string stepName) =>
            this.currentStep = this.CreateStep(stepName);

        public void Failed(Exception exception) =>
            this.reporter.Failed(this.currentStep, exception);

        public void Passed() =>
            this.reporter.Passed(this.currentStep);
